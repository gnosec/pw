...
describe('ValidationService', () => {
    ...
    describe('validatePasswordOptions()', () => {
        ...
        it('should validate length being too low', () => {
            expect(service.validatePasswordOptions({length: minimumLength - 1}).length).toBeGreaterThan(0);
        })

        it('should accept minimum length', () => {
            expect(service.validatePasswordOptions({length: minimumLength}).length).toBe(0);
        })

        it('should validate length being too high', () => {
            expect(service.validatePasswordOptions({length: maximumLength + 1}).length).toBeGreaterThan(0);
        })

        it('should accept maximum length', () => {
            expect(service.validatePasswordOptions({length: maximumLength}).length).toBe(0);
        })

        it('should validate charset', () => {
            expect(service.validatePasswordOptions({charset: 'not a charset'}).length).toBeGreaterThan(0);
        })

        it('should accept valid charset', () => {
            expect(service.validatePasswordOptions({charset: CharacterSetsByName.Numeric}).length).toBe(0);
        })
