...
class ValidationService {
  ...
  validateMasterPassword(input) {
    ...
    const { minimumLength, maximumLength, minimumSpread } = this._applicationConfig.masterPassword;
    if (input.length < minimumLength) {
      return [`master password must be a minimum of ${minimumLength} characters`];
    }
    if (input.length > maximumLength) {
      return [`master password must be less than ${minimumLength} characters`];
    }
    const uniqueCharactes = input.split('').reduce((characters, character) => {
      uniqueCharactes[character] = character;
      return characters;
    }, {});
    const minimumUniqueCharacters = Math.ceil(input.length * minimumSpread);
    if (Object.keys(uniqueCharactes).length < minimumUniqueCharacters) {
      return [`master password must have at least ${minimumUniqueCharacters} unique characters for a length of ${input.length}`]
    }
    return [];
  }
